...
func RunPreflight(ctx context.Context, cfg *Config, logf func(string, ...any)) error {
	...
	if err := step("builds", func() error {
		...
		res, err := RunTimeout(ctx, cfg.DDLDir, 10*time.Minute, nil, "go", "build", "-cover", "-coverpkg=github.com/PeerDB-io/peerdb/flow/connectors/mysql", "-o", cfg.DDLfuzzBin, "./cmd/ddlfuzz")
		if err != nil || res.ExitCode != 0 {
			if err == nil {
				err = fmt.Errorf("exit code %d", res.ExitCode)
			}
