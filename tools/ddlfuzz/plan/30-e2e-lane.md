...
4. **DDL under test**. On driver error → record `exec-reject` (append to
   `e2e-exec-rejects.jsonl` if generator/corpus-sourced, answer the queue if queue-sourced) and
   push `expect{kind:uncertain, caseID, text, driverError, best-effort before/after}` — the
   server may have applied and binlogged the statement despite the driver error, so the
   expectation admits 0 or 1 events. On success → **after-snapshot**, compute the ground-truth
...
binlogged-statement sequence, in order. The matcher aligns each event against worker k's FIFO
with a peek-commit walk — expectations are consumed only when a match commits, never
speculatively. Match rules: markers/controls pair on byte-equal text; an uncertain expectation
pairs on text equivalence; a ddl expectation pairs generically (no text equality — that is what
detects rewrites), except once the walk has passed a marker expectation, where a ddl match also
requires text equivalence. Markers are hard anchors: a marker event commits to its
caseID-matching expectation, and a non-marker event matching nothing ahead of an already-enqueued
marker expectation is provably extra (events are totally ordered; that marker's own event has
not arrived yet) → finding `e2e-unexpected-event`, FIFO untouched. Committed skips are
classified, never silent: skipped marker/control (their event missing — some servers do not
binlog a DROP TABLE IF EXISTS of a never-created table) and skipped uncertain (the normal
no-event exec-reject) → stats; a skipped ddl expectation with an empty delta is a benign no-op
that produced no event (e.g. comment-gated statements; queue answered `exec-reject`/no-event),
with a non-empty delta → finding `e2e-missing-event` (the server applied a schema change that
produced no binlog event) and queue answer `still-diverges`. An uncertain expectation matched by
an event → finding `e2e-exec-reject-applied` (driver error but server applied).

Pairing text policy: markers/controls pair only on byte-equal text (alignment above); a marker
event with no matching expectation anywhere in the FIFO is a **harness error** (crash loudly, do
not file a finding — the FIFO invariant is broken). For the DDL under test, byte-inequality of
...
If alignment is inconclusive (e.g. the FIFO is empty) when an event for `fuzz_w<k>` arrives, the
matcher blocks up to 30s for the worker to push (worker computes the after-snapshot before
pushing); timeout = harness error.
...
{
  "lane": "e2e",
  "class": "e2e-missed-column-effect",   // one of the classes defined in §4 + §6 + §8
  "engine": "mariadb",
...
- **Server-generated binlog events in fuzz schemas** (e.g. engine-internal DDL) desyncing
  pairing — markers bound the blast radius to one case; the event is filed loudly as
  `e2e-unexpected-event` (FIFO untouched) or caught at the next marker anchor (§4).
...
5. **New finding classes**: 00 doesn't enumerate classes. This plan defines
   ...
   `e2e-col-attr`, `e2e-position-missed`, the alignment-anomaly classes `e2e-missing-event`,
   `e2e-unexpected-event`, `e2e-exec-reject-applied` (§4), and `oracle-reject-live-accept`
   (filed via 20's replay). Component 20's descriptor function must incorporate the class string
   so e2e findings dedup separately from fast-lane ones.
