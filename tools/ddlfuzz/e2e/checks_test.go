...
func TestAlignExpectSkipsStaleDDLBeforeControl(t *testing.T) {
	q := newExpectQueue()
	q.Push(caseExpect{Kind: expectControl, Submitted: "DROP TABLE IF EXISTS `fixture`"})
	m := &matcher{ec: engineConfig{Name: EngineMySQL}}
	got, aligned := m.alignExpect(q, &replication.QueryEvent{
		Query: []byte("DROP TABLE IF EXISTS `fixture` /* generated by server */"),
	}, caseExpect{
		Kind:      expectDDL,
		Submitted: "/*!100000 SET STATEMENT max_statement_time=60 FOR ALTER TABLE fixture DROP COLUMN id */",
	})
	if !aligned {
		t.Fatal("aligned = false, want true")
	}
	if got.Kind != expectControl || got.Submitted != "DROP TABLE IF EXISTS `fixture`" {
		t.Fatalf("aligned expectation = %#v, want control drop", got)
	}
	if q.Len() != 0 {
