...
func (s PeerFlowE2ETestSuitePG) Test_NaN_Infinity_PG() {
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "initial load nan/inf", func() bool {
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "cdc nan/inf", func() bool {
		return s.comparePGTables(srcTableName, dstTableName, "id,f4::text,f8::text,n::text") == nil
	})
...
func (s PeerFlowE2ETestSuitePG) Test_Raw_Batch_Cleanup_PG() {
	...
	for round := range 5 {
		...
		e2e.EnvWaitFor(s.t, env, 3*time.Minute, fmt.Sprintf("normalize batch %d", expectedBatchID), func() bool {
	...
	e2e.EnvWaitFor(s.t, env, 1*time.Minute, "raw table cleanup", func() bool {
		var rawCount int64
		err := s.Conn().QueryRow(s.t.Context(),
...
func (s PeerFlowE2ETestSuitePG) Test_TransformRecordScript() {
	...
	e2e.InsertScript(s.t, "cdc_transform_record", "lua", "function transformRecord(r) if r.row then r.row.val = 1729 end end")
...
func (s PeerFlowE2ETestSuitePG) Test_TransformRowScript() {
	...
	e2e.InsertScript(s.t, "cdc_transform_row", "lua", "function transformRow(r) r.val = 1729 end")
...
func (s PeerFlowE2ETestSuitePG) TestResync(tableName string) {
	...
	env = e2e.ExecuteDropFlow(s.t.Context(), tc, flowConnConfig)
	e2e.EnvWaitForFinished(s.t, env, 3*time.Minute)
...
func (s PeerFlowE2ETestSuitePG) Test_Table_With_Excluded_PK_And_ReplicaIdentityFull() {
	...
	e2e.EnvWaitFor(s.t, env, 1*time.Minute, "normalize insert", func() bool {
	...
	e2e.EnvWaitFor(s.t, env, 1*time.Minute, "normalize update", func() bool {
...
func (s PeerFlowE2ETestSuitePG) Test_PG_PG_Target_Foreign_Keys() {
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "wait for child table initial load", func() bool {
	...
	e2e.EnvNoError(s.t, env, cdcTx.Commit(s.t.Context()))

	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "wait for child table cdc", func() bool {
		return s.comparePGTables(srcTableName2, dstTableName2, "id,parent_id,val") == nil
	})
