...
func (s PeerFlowE2ETestSuitePG) Test_Complete_QRep_Flow_Multi_Insert_PG() {
	...
	jobName := e2e.AddSuffix(s, baseName)
...
func (s PeerFlowE2ETestSuitePG) Test_QRep_Failed_Run_Marked_In_Catalog() {
	...
	require.NoError(s.t, e2e.CreateTableForQRep(s.t.Context(), s.Conn(), s.suffix, dstTable))
...
func (s PeerFlowE2ETestSuitePG) TestQRepPause() {
	...
	jobName := e2e.AddSuffix(s, srcTable)
...
func (s PeerFlowE2ETestSuitePG) TestTransform() {
	...
	e2e.InsertScript(s.t, "pgtransform", "lua", "function transformRow(row) row.myreal = 1729 end")
