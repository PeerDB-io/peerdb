...
func (s PeerFlowE2ETestSuitePG) Test_PG_Schema_Dump_And_CDC() {
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "initial load parent", func() bool {
		var count int64
		err := dstConn.QueryRow(s.t.Context(),
			"SELECT COUNT(*) FROM "+dstParent).Scan(&count)
		return err == nil && count == 5
	})
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "initial load child", func() bool {
		var count int64
		err := dstConn.QueryRow(s.t.Context(),
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "cdc parent rows", func() bool {
		var count int64
		err := dstConn.QueryRow(s.t.Context(),
			"SELECT COUNT(*) FROM "+dstParent).Scan(&count)
		return err == nil && count == 8
	})
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "cdc child rows", func() bool {
		var count int64
		err := dstConn.QueryRow(s.t.Context(),
			"SELECT COUNT(*) FROM "+dstChild).Scan(&count)
...
func (s PeerFlowE2ETestSuitePG) Test_PG_Schema_Dump_No_Owner_No_Privileges() {
	...
	e2e.CreatePeer(s.t, &protos.Peer{
		Name:   dstPeerName,
		Type:   protos.DBType_POSTGRES,
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "initial load owned_tbl", func() bool {
		var count int64
		err := dstConn.QueryRow(s.t.Context(),
	...
	e2e.EnvWaitFor(s.t, env, 3*time.Minute, "cdc owned_tbl", func() bool {
		var count int64
		err := dstConn.QueryRow(s.t.Context(),
			"SELECT COUNT(*) FROM "+qualified).Scan(&count)
		return err == nil && count == 10
