...
func (s ClickHouseSuite) Test_UnsignedMySQL() {
	...
	srcTable := s.attachSchemaSuffix(srcTableName)
	srcFullName := srcTable.Deparse()
	...
	`, srcTable.MySQL(), int64(math.MinInt64), uint64(math.MaxUint64))))
...
func (s ClickHouseSuite) Test_MySQL_Bit() {
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(), fmt.Sprintf(`INSERT INTO %s ("key",b1,b20) VALUES
		('init',b'1',b'11100011100011100011'), ('init',b'0',b'00011100011100011100')`, srcTable.MySQL())))
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(), fmt.Sprintf(`INSERT INTO %s ("key",b1,b20) VALUES
		('cdc','1','11100011100011100011'), ('cdc','0','00011100011100011100')`, srcTable.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_Blobs() {
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(), fmt.Sprintf(`INSERT INTO %s (k,tb,mb,lb,bi,vb,tt,mt,lt,ch,vc,js) VALUES
		...
		'tinytext','mediumtext','longtext','char','varchar','{"a":2}')`, srcTable.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_Enum() {
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(), fmt.Sprintf(`INSERT INTO %s ("key",e,s) VALUES
		('init','b''s','a,b'),('init','','')`, srcTable.MySQL())))
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(), fmt.Sprintf(`INSERT INTO %s ("key",e,s) VALUES
		('cdc','b''s','a,b'),('cdc','','')`, srcTable.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_Vector() {
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(),
		fmt.Sprintf(`INSERT INTO %s (val) VALUES (to_vector('[1.1,1.0]'))`, srcTable.MySQL())))
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(),
		fmt.Sprintf(`INSERT INTO %s (val) VALUES (to_vector('[2.718, 1.414]'))`, srcTable.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_Geometric_Types() {
	...
	srcFullName := s.attachSchemaSuffix(srcTableName).Deparse()
...
func (s ClickHouseSuite) Test_MySQL_Schema_Changes() {
	...
	srcTableQT := AttachSchema(s, srcTable)
	...
	secondSrcTableQT := AttachSchema(s, secondSrcTable)
	...
	EnvNoError(t, env, s.Source().Exec(t.Context(), fmt.Sprintf(`INSERT INTO %s(c1) VALUES(1)`, srcTableQT.MySQL())))
	...
	EnvNoError(t, env, s.Source().Exec(t.Context(), fmt.Sprintf("ALTER TABLE %s ADD COLUMN `addedColumn` BIGINT", srcTableQT.MySQL())))
	// so that the batch finishes, insert a row into the second source table.
	EnvNoError(t, env, s.Source().Exec(t.Context(), fmt.Sprintf(`INSERT INTO %s VALUES(DEFAULT)`, secondSrcTableQT.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_GhOst_Schema_Changes() {
	...
	ghostTableQT := AttachSchema(s, ghostTable)
	oldTable := "_" + srcTable + "_del"
	oldTableQT := AttachSchema(s, oldTable)
	...
	connectionGen := FlowConnectionGenerationConfig{
		FlowJobName:      AddSuffix(s, srcTable),
		TableNameMapping: map[string]string{srcTableQT.Deparse(): dstTable},
		Destination:      s.Peer().Name,
	}
	flowConnConfig := connectionGen.GenerateFlowConnectionConfigs(s)
	...
	`, ghostTableQT.MySQL(), srcTableQT.MySQL())))

	// 3. Insert another row into original table (gh-ost would capture this via binlog and apply to ghost)
	EnvNoError(t, env, s.Source().Exec(t.Context(), fmt.Sprintf(`INSERT INTO %s(c1) VALUES(2)`, srcTableQT.MySQL())))
	...
	`, srcTableQT.MySQL(), oldTableQT.MySQL(), ghostTableQT.MySQL(), srcTableQT.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_NumToVarcharCoercion() {
	...
	require.NoError(s.t, s.source.Exec(s.t.Context(),
		fmt.Sprintf(`INSERT INTO %s(num,f32,f64)VALUES(780780780,1.41421,2.718281828459045)`, srcTable.MySQL())))
...
func (s ClickHouseSuite) Test_MySQL_Column_Position_Shifting_DDL_Error() {
	...
	for _, tc := range testCases {
		s.t.Run(tc.name, func(t *testing.T) {
			...
			srcTableQT := s.attachSchemaSuffix(srcTableName)
			srcFullName := srcTableQT.Deparse()
			dstTableName := fmt.Sprintf("test_position_shift_%s_dst", tc.name)

			require.NoError(t, s.source.Exec(s.t.Context(), fmt.Sprintf(
				`CREATE TABLE IF NOT EXISTS %s (id SERIAL PRIMARY KEY, c1 INT)`, srcTableQT.MySQL())))

			require.NoError(t, s.source.Exec(s.t.Context(), fmt.Sprintf(
				`INSERT INTO %s (c1) VALUES (1)`, srcTableQT.MySQL())))
			...
			require.NoError(t, s.source.Exec(s.t.Context(), fmt.Sprintf(
				`ALTER TABLE %s %s`, srcTableQT.MySQL(), tc.ddlSQL)))
