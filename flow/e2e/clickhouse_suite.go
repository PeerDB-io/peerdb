...
// Postgres 15+ allows a negative numeric scale. ClickHouse has no Decimal with a negative scale,
// so numeric(5,-2) must map to the default Decimal(76, 38) and the destination table must be created.
func (s ClickHouseSuite) Test_Numeric_Negative_Scale() {
	pgSource, ok := s.source.(*PostgresSource)
	if !ok {
		s.t.Skip("only applies to postgres")
	}
	var serverVersion int
	require.NoError(s.t, pgSource.Conn().QueryRow(s.t.Context(),
		"SELECT current_setting('server_version_num')::int").Scan(&serverVersion))
	if serverVersion < 150000 {
		s.t.Skip("negative numeric scale requires postgres 15+")
	}

	srcTableName := "numeric_negative_scale"
	srcFullName := s.attachSchemaSuffix(srcTableName)
	...
	`, srcFullName)))
	// numeric(5,-2) rounds to hundreds: 12345 is stored as 12300
	require.NoError(s.t, s.source.Exec(s.t.Context(),
		fmt.Sprintf("INSERT INTO %s(n) VALUES (12345), (-9999949)", srcFullName)))

	connectionGen := FlowConnectionGenerationConfig{
		FlowJobName:      s.attachSuffix("ch_numeric_neg_scale"),
		TableNameMapping: map[string]string{srcFullName: dstTableName},
		Destination:      s.Peer().Name,
	}
	flowConnConfig := connectionGen.GenerateFlowConnectionConfigs(s)
	...
	EnvWaitForCount(env, s, "waiting on initial load", dstTableName, "id,n", 2)

	require.NoError(s.t, s.source.Exec(s.t.Context(),
		fmt.Sprintf("INSERT INTO %s(n) VALUES (9999900)", srcFullName)))

	EnvWaitForCount(env, s, "waiting on cdc", dstTableName, "id,n", 3)
	...
	require.NoError(s.t, ch.QueryRow(s.t.Context(), fmt.Sprintf(
		"select type from system.columns where database=%s and table=%s and name='n'",
		clickhouse.QuoteLiteral(s.connector.Config.Database),
		clickhouse.QuoteLiteral(dstTable),
	)).Scan(&columnType))
	require.Equal(s.t, "Decimal(76, 38)", columnType)

	rows, err := s.GetRows(dstTableName, "id,n")
	require.NoError(s.t, err)
	require.Len(s.t, rows.Records, 3)
	for i, expected := range []string{"12300", "-9999900", "9999900"} {
		n, ok := rows.Records[i][1].Value().(decimal.Decimal)
		require.True(s.t, ok, "expected numeric(5,-2) to be Decimal")
		require.Equal(s.t, expected, n.String())
	}
...
func (s ClickHouseSuite) Test_Types_CH() {
	...
	if err := s.Source().Exec(s.t.Context(), createMoodEnum); err != nil &&
