...
func (s EventhubsSuite) Teardown(ctx context.Context) {
	...
	err = e2e.DeleteEventhub(ctx, s.GetEventhubName(), creds)
	require.NoError(s.t, err)
}
...
func (s EventhubsSuite) Test_EH_Simple() {
	...
	e2e.InsertScript(s.t, "e2e_eh_simple_script", "lua", "function onRecord(r) return r.row and r.row.val end")
	...
	e2e.EnvWaitFor(s.t, env, 2*time.Minute, "eventhubs message check", func() bool {
		messages, err := e2e.ConsumeAllMessages(
			s.t.Context(),
			ehCreds.Namespace,
			s.GetEventhubName(),
			1,
