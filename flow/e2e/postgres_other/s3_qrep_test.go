...
func setupSuite(t *testing.T, s3environment e2e.S3Environment) PeerFlowE2ETestSuiteS3 {
	t.Helper()
	...
	helper, err := e2e.NewS3TestHelper(t.Context(), s3environment)
	if err != nil {
...
func (s PeerFlowE2ETestSuiteS3) Test_Complete_QRep_Flow_S3() {
	...
	jobName := e2e.AddSuffix(s, tableName)
	schemaQualifiedName := e2e.AttachSchema(s, tableName)
