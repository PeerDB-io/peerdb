...
func (s PeerFlowE2ETestSuiteSF) Test_Numeric_Truncation_SF() {
	tc := e2e.NewTemporalClient(s.t)
	srcTableName := s.attachSchemaSuffix("test_numeric_truncation_sf")
	dstTableName := fmt.Sprintf("%s.%s", s.sfHelper.testSchemaName, "test_numeric_truncation_sf")
	...
	tests := []struct {
		...
		{SrcType: "numeric", SrcValue: nines(18, 20), Expected: nines(18, 20)},
		{SrcType: "numeric", SrcValue: nines(19, 0), Expected: "0"},
		{SrcType: "numeric", SrcValue: nines(0, 21), Expected: nines(0, 20)},
	...
	connectionGen := e2e.FlowConnectionGenerationConfig{
		FlowJobName:      s.attachSuffix("test_numeric_truncation_sf"),
		TableNameMapping: map[string]string{srcTableName: dstTableName},
		Destination:      s.Peer().Name,
	}
	flowConnConfig := connectionGen.GenerateFlowConnectionConfigs(s)
