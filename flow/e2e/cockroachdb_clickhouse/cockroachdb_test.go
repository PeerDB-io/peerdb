...
package cockroachdb_clickhouse
...
func (s CockroachDBSuite) Source() e2e.SuiteSource {
	return s.source
}

func (s CockroachDBSuite) Connector() *conncockroachdb.CockroachDBConnector {
	return s.source.CockroachDBConnector()
}
...
func SetupCockroachDBSuite(t *testing.T) CockroachDBSuite {
	...
	source, err := e2e.SetupCockroachDB(t, suffix)
...
func setupCockroachDBTypesTable(s e2e.Suite, table string) {
	t := s.T()
	t.Helper()
	ctx := t.Context()
	schema := e2e.Schema(s)
...
func TestCockroachDBRangefeedValidation(t *testing.T) {
	...
	source, err := e2e.SetupCockroachDBWithConfig(t, suffix, e2e.CockroachDBTestContainerConfig(t))
	...
	require.ErrorContains(t, source.CockroachDBConnector().ValidateMirrorSource(ctx, cdcConfig), "rangefeed")
	require.NoError(t, source.CockroachDBConnector().ValidateMirrorSource(ctx, snapshotOnly))
	require.NoError(t, source.Exec(ctx, "SET CLUSTER SETTING kv.rangefeed.enabled = true"))
	require.NoError(t, source.CockroachDBConnector().ValidateMirrorSource(ctx, cdcConfig))
}
...
func (s CockroachDBSuite) Test_GC_TTL_Validation() {
	...
	// the default zone config (25 hours) is above the floor
	require.NoError(t, s.Connector().ValidateMirrorSource(ctx, cfg))

	// below the floor mirrors are rejected
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		"ALTER TABLE %s.gcttl_check CONFIGURE ZONE USING gc.ttlseconds = 3600", schema)))
	err := s.Connector().ValidateMirrorSource(ctx, cfg)
	require.ErrorContains(t, err, "gc.ttlseconds")
	require.ErrorContains(t, err, "below the 86400 second (24 hour) minimum")

	// exactly at the floor passes again
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		"ALTER TABLE %s.gcttl_check CONFIGURE ZONE USING gc.ttlseconds = 86400", schema)))
	require.NoError(t, s.Connector().ValidateMirrorSource(ctx, cfg))

	// the floor also guards CDC mirrors without an initial snapshot: a paused
	// mirror's resume cursor dies with the GC window just like snapshot reads
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		"ALTER TABLE %s.gcttl_check CONFIGURE ZONE USING gc.ttlseconds = 3600", schema)))
	noSnapshot := proto.CloneOf(cfg)
	noSnapshot.DoInitialSnapshot = false
	noSnapshot.InitialSnapshotOnly = false
	require.ErrorContains(t, s.Connector().ValidateMirrorSource(ctx, noSnapshot),
		"below the 86400 second (24 hour) minimum")
...
func (s CockroachDBSuite) Test_GC_TTL_Validation_Partitions() {
	t := s.t
	ctx := t.Context()
	schema := "e2e_test_" + s.suffix

	// partition zone configs override the table level, so a low TTL on one
	// partition must fail validation even when the table itself is fine
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		`CREATE TABLE %s.gcttl_part (region TEXT, id INT8, v TEXT, PRIMARY KEY (region, id))
		PARTITION BY LIST (region) (
			PARTITION hot VALUES IN ('us-east'),
			PARTITION cold VALUES IN (DEFAULT)
		)`, schema)))
	...
	cfg := &protos.FlowConnectionConfigsCore{
		...
		InitialSnapshotOnly: true,
		TableMappings: []*protos.TableMapping{{
			SourceTableIdentifier:      schema + ".gcttl_part",
			DestinationTableIdentifier: "gcttl_part",
		}},
	}
	require.NoError(t, s.Connector().ValidateMirrorSource(ctx, cfg))

	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		"ALTER PARTITION cold OF TABLE %s.gcttl_part CONFIGURE ZONE USING gc.ttlseconds = 600", schema)))
	err := s.Connector().ValidateMirrorSource(ctx, cfg)
	require.ErrorContains(t, err, "below the 86400 second (24 hour) minimum")
	require.ErrorContains(t, err, "PARTITION cold OF TABLE")

	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		"ALTER PARTITION cold OF TABLE %s.gcttl_part CONFIGURE ZONE USING gc.ttlseconds = 86400", schema)))
	require.NoError(t, s.Connector().ValidateMirrorSource(ctx, cfg))
}
...
func (s CockroachDBSuite) Test_History_Retention_Protection() {
	...
	// the flow name is deliberately far longer than the ~69 characters at
	// which SHOW JOBS middle-truncates job descriptions: protect, extend and
	// release must keep working for names the truncated description can no
	// longer identify (they resolve jobs through crdb_internal.jobs, which
	// carries the full description)
	flowName := "hr_" + s.suffix + "_" + strings.Repeat("long_flow_name_", 6)
	require.GreaterOrEqual(t, len(flowName), 80)

	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(
		"CREATE TABLE %s.hr_check (id INT8 PRIMARY KEY, v TEXT)", schema)))

	cfConn, err := conncockroachdb.NewCockroachDBConnector(ctx, nil, proto.CloneOf(s.source.Config()))
	require.NoError(t, err)
	defer func() {
		require.NoError(t, cfConn.Close())
	}()

	// v26.1+ restricts crdb_internal reads behind this session variable;
	// earlier versions either default it to allow or do not know it
	...
	countRunning := func() int {
		...
		require.NoError(t, s.source.AdminConn().QueryRow(ctx,
			"SELECT count(*) FROM crdb_internal.jobs WHERE job_type = 'HISTORY RETENTION'"+
	...
	var jobID int64
	require.NoError(t, s.source.AdminConn().QueryRow(ctx,
		"SELECT job_id FROM crdb_internal.jobs WHERE job_type = 'HISTORY RETENTION'"+
			" AND status = 'running' AND right(description, length($1::text)) = $1::text LIMIT 1",
		"peerdb initial load "+flowName).Scan(&jobID))

	// pin the premise: SHOW JOBS truncates this job's description, so a
	// suffix match against it cannot find the job (the historical bug made
	// extend and release silent no-ops for long flow names). If this ever
	// fails, SHOW JOBS stopped truncating and the crdb_internal.jobs
	// indirection is no longer load-bearing.
	var showJobsMatches int
	require.NoError(t, s.source.AdminConn().QueryRow(ctx,
		"SELECT count(*) FROM [SHOW JOBS] WHERE job_type = 'HISTORY RETENTION'"+
			" AND status = 'running' AND right(description, length($1::text)) = $1::text",
		"peerdb initial load "+flowName).Scan(&showJobsMatches))
	require.Equal(t, 0, showJobsMatches,
		"SHOW JOBS should truncate the description of a long flow name; the job must be found via crdb_internal.jobs instead")
	...
	// the job is matched by mirror name: cleanup of another mirror must not
	// cancel this mirror's protection
	require.NoError(t, s.Connector().PullFlowCleanup(ctx, flowName+"_other"))
	require.Equal(t, 1, countRunning(), "cleanup of another mirror should leave the retention job alone")
...
func (s CockroachDBSuite) Test_Peer_Creation_And_Validate() {
	...
	require.NoError(t, s.Connector().ConnectionActive(ctx), "cockroachdb connection should be active")
	require.NoError(t, s.Connector().ValidateCheck(ctx), "cockroachdb should pass version gate")
}
...
func (s CockroachDBSuite) Test_Schema_Introspection() {
	...
	schemasResp, err := s.Connector().GetSchemas(ctx)
	require.NoError(t, err)
	require.Contains(t, schemasResp.Schemas, e2e.Schema(s))

	tablesResp, err := s.Connector().GetTablesInSchema(ctx, e2e.Schema(s), false)
	require.NoError(t, err)
	...
	allTablesResp, err := s.Connector().GetAllTables(ctx)
	require.NoError(t, err)
	require.Contains(t, allTablesResp.Tables, e2e.AttachSchema(s, srcTable))

	columnsResp, err := s.Connector().GetColumns(ctx, shared.InternalVersion_Latest, e2e.Schema(s), srcTable)
	require.NoError(t, err)
...
func (s CockroachDBSuite) Test_Table_Schema_QValue_Kinds() {
	...
	tableMappings := []*protos.TableMapping{{
		SourceTableIdentifier:      e2e.AttachSchema(s, srcTable),
	...
	tableSchema := schemas[e2e.AttachSchema(s, srcTable)]
	...
	require.NoError(t, err)
	require.True(t, nullableSchemas[e2e.AttachSchema(s, srcTable)].NullableEnabled,
		"PEERDB_NULLABLE should propagate to the table schema")
}

func (s CockroachDBSuite) Test_Decimal_Type_Modifiers() {
	t := s.t
	ctx := t.Context()
	schema := "e2e_test_" + s.suffix
	table := "decimal_typmods"

	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf(`CREATE TABLE %s.%s (
		id INT8 PRIMARY KEY,
		d_full DECIMAL(10,2),
		d_precision_only DECIMAL(10),
		d_bare DECIMAL
	)`, schema, table)))
	...
	typmods := make(map[string]int32, len(schemas[schema+"."+table].Columns))
	for _, col := range schemas[schema+"."+table].Columns {
		typmods[col.Name] = col.TypeModifier
	}
	// DECIMAL(p) means DECIMAL(p,0) and the server reports scale 0 for it, so
	// precision only declarations keep their typmod on the way to destinations
	require.Equal(t, datatypes.MakeNumericTypmod(10, 2), typmods["d_full"])
	require.Equal(t, datatypes.MakeNumericTypmod(10, 0), typmods["d_precision_only"])
	require.Equal(t, int32(-1), typmods["d_bare"], "unbounded DECIMAL should stay -1")
}
...
// changefeedConnector builds a dedicated connector for a CDC test, so its
// changefeed sessions and cleanup never interfere with the suite's shared
// connector.
func (s CockroachDBSuite) changefeedConnector(t *testing.T) *conncockroachdb.CockroachDBConnector {
	t.Helper()
	cfConn, err := conncockroachdb.NewCockroachDBConnector(t.Context(), nil, proto.CloneOf(s.source.Config()))
	require.NoError(t, err)
...
func (s CockroachDBSuite) Test_CDC_Live_PullRecords() {
	...
	require.NoError(t, err)
	require.GreaterOrEqual(t, len(records), 6, "3 inserts + 1 update + 1 delete + 1 mixed-case insert")

	// the connector passes duplicates through (at-least-once delivery, e.g.
	// after a mid-batch reconnect while a parallel test flips rangefeeds), so
	// events are asserted as a set: every expected event arrives, nothing
	// unexpected does, and duplicates of expected events are tolerated
	type cdcEvent struct {
		dst  string
		id   int64
		kind string
	}
	recordEvent := func(record model.Record[model.RecordItems]) cdcEvent {
		switch typed := record.(type) {
		case *model.InsertRecord[model.RecordItems]:
			return cdcEvent{typed.DestinationTableName, typed.Items.GetColumnValue("id").(types.QValueInt64).Val, "insert"}
		case *model.UpdateRecord[model.RecordItems]:
			return cdcEvent{typed.DestinationTableName, typed.NewItems.GetColumnValue("id").(types.QValueInt64).Val, "update"}
		case *model.DeleteRecord[model.RecordItems]:
			return cdcEvent{typed.DestinationTableName, typed.Items.GetColumnValue("id").(types.QValueInt64).Val, "delete"}
		default:
			t.Fatalf("unexpected record type %T", record)
			return cdcEvent{}
		}
	}
	expectedEvents := []cdcEvent{
		{"cdc_rows_dst", 1, "insert"},
		{"cdc_rows_dst", 2, "insert"},
		{"cdc_rows_dst", 3, "insert"},
		{"cdc_rows_dst", 2, "update"},
		{"cdc_rows_dst", 3, "delete"},
		{"cdc_mixed_dst", 10, "insert"},
	}
	...
	for _, record := range records {
		...
		switch typed := record.(type) {
		case *model.InsertRecord[model.RecordItems]:
			if event == (cdcEvent{"cdc_rows_dst", 1, "insert"}) {
	...
	for _, want := range expectedEvents {
		require.GreaterOrEqual(t, seen[want], 1, "expected event %v must arrive at least once", want)
	}
	for got := range seen {
		require.Contains(t, expectedEvents, got, "only expected events may arrive")
	}
	require.GreaterOrEqual(t, seen[cdcEvent{"cdc_mixed_dst", 10, "insert"}], 1,
		"mixed-case quoted table must route through live full_table_name")
	...
	// arrive, and nothing outside the expected set may show up. Rows from the
	// first batch's tail may be re-delivered (at-least-once behavior on batch
	// boundaries and reconnects; destinations converge), but nothing may be
	// lost.
	...
	require.GreaterOrEqual(t, newRows["cdc_rows_dst/4"], 1,
		"rows written after the checkpoint must not be lost")
	require.GreaterOrEqual(t, newRows["cdc_mixed_dst/11"], 1,
		"rows written after the checkpoint must not be lost")
...
func (s CockroachDBSuite) Test_CDC_Schema_Delta_At_Event_Time() {
	...
	ctx := t.Context()
	schema := "e2e_test_" + s.suffix
	flowName := "cdcdelta_" + s.suffix
	src := schema + ".delta_check"
	...
	_, err := cfConn.SetupReplication(ctx, shared.CatalogPool{}, &protos.SetupReplicationInput{
		FlowJobName:      flowName,
		TableNameMapping: map[string]string{src: "delta_dst"},
	})
	require.NoError(t, err)
	lastOffset, err := cfConn.GetLastOffset(ctx, flowName)
	require.NoError(t, err)
	...
	schemas, err := cfConn.GetTableSchema(ctx, nil, shared.InternalVersion_Latest, protos.TypeSystem_Q,
		[]*protos.TableMapping{{SourceTableIdentifier: src, DestinationTableIdentifier: "delta_dst"}})
	require.NoError(t, err)

	// the cached schema predates the DDL: a column is added, a row is written
	// with it and the column is dropped again before the pull. The schema
	// delta must still type the column correctly, which requires reading the
	// source schema AS OF SYSTEM TIME of the row instead of the latest one.
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf("INSERT INTO %s VALUES (1, 'a')", src)))
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf("ALTER TABLE %s ADD COLUMN extra INT8", src)))
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf("INSERT INTO %s (id, v, extra) VALUES (2, 'b', 42)", src)))
	require.NoError(t, s.source.Exec(ctx, fmt.Sprintf("ALTER TABLE %s DROP COLUMN extra", src)))
	...
	records, _, err := pullOneBatch(t, ctx, cfConn, req)
	require.NoError(t, err)

	var sawExtra bool
	for _, record := range records {
		insert, ok := record.(*model.InsertRecord[model.RecordItems])
		if !ok {
			continue
		}
		if insert.Items.GetColumnValue("id") == (types.QValueInt64{Val: 2}) {
			require.Equal(t, types.QValueInt64{Val: 42}, insert.Items.GetColumnValue("extra"),
				"the added column must be typed by the schema at the row's commit timestamp")
			sawExtra = true
		}
	}
	require.True(t, sawExtra, "the row written with the added column must arrive")

	deltas := req.RecordStream.SchemaDeltas
	require.Len(t, deltas, 1)
	require.Len(t, deltas[0].AddedColumns, 1)
	require.Equal(t, "extra", deltas[0].AddedColumns[0].Name)
	require.Equal(t, string(types.QValueKindInt64), deltas[0].AddedColumns[0].Type)
}
...
func SetupCockroachDBClickHouseSuite(t *testing.T) CockroachDBClickHouseSuite {
	t.Helper()
	return CockroachDBClickHouseSuite{e2e.SetupClickHouseSuite(t, false, func(t *testing.T) (*e2e.CockroachDBSource, string, error) {
		t.Helper()
...
func (s CockroachDBClickHouseSuite) Test_CDC_Simple() {
	...
	srcQualified := e2e.AttachSchema(s, srcTable)
	...
	e2e.EnvWaitForEqualTablesWithNames(env, s, "initial snapshot loaded", srcTable, dstTable, "id,ky,val")

	for i := range 5 {
		e2e.EnvNoError(t, env, s.Source().Exec(ctx, fmt.Sprintf(
			"INSERT INTO %s (id, ky, val) VALUES (%d, 'cdc_key_%d', 'cdc_value_%d')", srcQualified, i+6, i, i)))
	}
	e2e.EnvNoError(t, env, s.Source().Exec(ctx, fmt.Sprintf("UPDATE %s SET val = 'updated' WHERE id = 2", srcQualified)))
	e2e.EnvNoError(t, env, s.Source().Exec(ctx, fmt.Sprintf("DELETE FROM %s WHERE id = 3", srcQualified)))

	e2e.EnvWaitForEqualTablesWithNames(env, s, "normalizing cdc inserts, an update and a delete",
...
func (s CockroachDBClickHouseSuite) Test_QRep_Simple() {
	...
	qrepConfig := e2e.CreateQRepWorkflowConfig(t,
		e2e.AddSuffix(s, srcTable),
		srcQualified,
		dstTable,
	...
	tc := e2e.NewTemporalClient(t)
	env := e2e.RunQRepFlowWorkflow(t, tc, qrepConfig)
...
func (s CockroachDBClickHouseSuite) Test_QRep_Types() {
	...
	e2e.RequireEqualTablesWithNames(s, srcTable, dstTable,
