...
func (s SwitchboardMySQLSuite) T() *testing.T {
	return s.t
}

func (s SwitchboardMySQLSuite) Teardown(ctx context.Context) {
...
func setupSwitchboardMySQLFamilySuite(
	...
	// Set the database in the config so Switchboard connects to it
	...
	conn, err := pgx.Connect(t.Context(), e2e.SwitchboardDSN(peer.Name, nil))
	if err != nil {
...
func (s SwitchboardMySQLSuite) queryWithOptions(sql string, options map[string]string) error {
	dsn := e2e.SwitchboardDSN(s.peer.Name, options)
...
func (s SwitchboardMySQLSuite) psql(sql string) (string, error) {
	return e2e.RunPsql(s.t, s.peer.Name, "-tA", "-c", sql)
}

// testTable returns the fully qualified test table name
func (s SwitchboardMySQLSuite) testTable() string {
...
func (s SwitchboardMySQLSuite) Test_BasicConnectivity_SimpleSelect() {
	output, err := s.psql("SELECT 1")
	require.NoError(s.t, err)
	require.Equal(s.t, "1", output)
}

func (s SwitchboardMySQLSuite) Test_BasicConnectivity_SelectVersion() {
	output, err := s.psql("SELECT VERSION()")
	require.NoError(s.t, err)
	require.NotEmpty(s.t, output)
}

func (s SwitchboardMySQLSuite) Test_BasicConnectivity_CurrentDatabase() {
...
func (s SwitchboardMySQLSuite) Test_DataTypes() {
	tests := []struct {
		name     string
		sql      string
		expected string
...
func (s SwitchboardMySQLSuite) Test_QueryComplexity_MultipleRows() {
...
func (s SwitchboardMySQLSuite) Test_QueryComplexity_MultipleColumns() {
...
func (s SwitchboardMySQLSuite) Test_QueryComplexity_SelfJoin() {
...
func (s SwitchboardMySQLSuite) Test_QueryComplexity_Subquery() {
	output, err := s.psql("SELECT * FROM (SELECT 1 AS x) AS sub")
	require.NoError(s.t, err)
	require.Equal(s.t, "1", output)
}

func (s SwitchboardMySQLSuite) Test_QueryComplexity_SubqueryInWhere() {
	output, err := s.psql(fmt.Sprintf(
...
func (s SwitchboardMySQLSuite) Test_QueryComplexity_Aggregates() {
	output, err := s.psql("SELECT COUNT(*) FROM " + s.testTable())
	require.NoError(s.t, err)
	require.Equal(s.t, "3", output)
}

func (s SwitchboardMySQLSuite) Test_QueryComplexity_GroupBy() {
	output, err := s.psql(fmt.Sprintf(
...
func (s SwitchboardMySQLSuite) Test_QueryComplexity_UnionAll() {
...
func (s SwitchboardMySQLSuite) Test_Desc_Table() {
	output, err := s.psql("DESC " + s.testTable())
	require.NoError(s.t, err)
	require.Contains(s.t, output, "id")
}

func (s SwitchboardMySQLSuite) Test_Explain_Select() {
...
func (s SwitchboardMySQLSuite) Test_Transaction_BeginCommit() {
	dsn := e2e.SwitchboardDSN(s.peer.Name, nil)
...
func (s SwitchboardMySQLSuite) Test_Transaction_BeginRollback() {
	dsn := e2e.SwitchboardDSN(s.peer.Name, nil)
	cfg, _ := pgx.ParseConfig(dsn)
	cfg.DefaultQueryExecMode = pgx.QueryExecModeSimpleProtocol
	conn, err := pgx.ConnectConfig(s.t.Context(), cfg)
...
func (s SwitchboardMySQLSuite) Test_Transaction_Savepoint() {
	dsn := e2e.SwitchboardDSN(s.peer.Name, nil)
	cfg, _ := pgx.ParseConfig(dsn)
	cfg.DefaultQueryExecMode = pgx.QueryExecModeSimpleProtocol
	conn, err := pgx.ConnectConfig(s.t.Context(), cfg)
...
func (s SwitchboardMySQLSuite) Test_BlockedStatements() {
	tests := []struct {
		name string
...
func (s SwitchboardMySQLSuite) Test_DeniedFunctions() {
	tests := []struct {
		name string
		sql  string
	}{
...
func (s SwitchboardMySQLSuite) Test_SelectIntoBlocked() {
...
func (s SwitchboardMySQLSuite) Test_SelectIntoParseFailures() {
...
func (s SwitchboardMySQLSuite) Test_LockingReadsBlocked() {
	tests := []struct {
		name string
		sql  string
	}{
...
func (s SwitchboardMySQLSuite) Test_Explain_SelectAllowed() {
	_, err := s.psql("EXPLAIN SELECT 1")
	require.NoError(s.t, err)
}

func (s SwitchboardMySQLSuite) Test_ExplainNonSelectBlocked() {
	tests := []struct {
		name string
		sql  string
	}{
...
func (s SwitchboardMySQLSuite) Test_Error_SyntaxError() {
	_, err := s.psql("SELCT 1")
	require.Error(s.t, err)
}

func (s SwitchboardMySQLSuite) Test_Error_TableNotExists() {
	output, err := s.psql("SELECT * FROM nonexistent_table_xyz")
	require.Error(s.t, err)
	require.Contains(s.t, output, "exist")
}

func (s SwitchboardMySQLSuite) Test_Error_ColumnNotExists() {
	output, err := s.psql("SELECT bad_column FROM " + s.testTable())
	require.Error(s.t, err)
	require.Contains(s.t, output, "column")
}

func (s SwitchboardMySQLSuite) Test_Error_PsqlBackslashD() {
...
func (s SwitchboardMySQLSuite) Test_Guardrails_RowLimitExceeded() {
...
func (s SwitchboardMySQLSuite) Test_Guardrails_WithinRowLimit() {
	err := s.queryWithOptions(
...
func (s SwitchboardMySQLSuite) Test_Guardrails_ByteLimitExceeded() {
	err := s.queryWithOptions(
...
func (s SwitchboardMySQLSuite) Test_EmptyQuery() {
	tests := []struct {
		name string
		sql  string
	}{
...
func (s SwitchboardMySQLSuite) Test_MidBatchError() {
...
func (s SwitchboardMySQLSuite) Test_CancelRequest() {
...
func (s SwitchboardMySQLSuite) Test_ConnectionUsableAfterLimitExceeded() {
	dsn := e2e.SwitchboardDSN(s.peer.Name, map[string]string{"max_rows": "3"})
...
func (s SwitchboardMySQLSuite) Test_Connection_Reuse() {
	dsn := e2e.SwitchboardDSN(s.peer.Name, nil)
	cfg, _ := pgx.ParseConfig(dsn)
	cfg.DefaultQueryExecMode = pgx.QueryExecModeSimpleProtocol
	conn, err := pgx.ConnectConfig(s.t.Context(), cfg)
...
func (s SwitchboardMySQLSuite) Test_ConcurrentConnections() {
	const numConns = 5
...
func (s SwitchboardMySQLSuite) Test_MultipleSequentialConnections() {
	for i := range 10 {
...
func (s SwitchboardMySQLSuite) Test_IdleTimeout_Short() {
	dsn := e2e.SwitchboardDSN(s.peer.Name, map[string]string{"idle_timeout": "1"})
