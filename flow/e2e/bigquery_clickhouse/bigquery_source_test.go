...
func bigQueryTestStagingPath(s e2e.Suite, path string) string {
	return fmt.Sprintf("%s/%s", stagingTestBucket, e2e.AddSuffix(s, path))
}
...
func SetupBigQueryClickhouseSuite(t *testing.T) BigQueryClickhouseSuite {
	t.Helper()
	return BigQueryClickhouseSuite{e2e.SetupClickHouseSuite(t, false, func(t *testing.T) (*bigQuerySource, string, error) {
		t.Helper()
...
func setupBigQuerySource(t *testing.T) (*bigQuerySource, error) {
	...
	helper, err := e2e.NewBigQueryTestHelper(t, "do_not_remove_peerdb_source_test_dataset")
	if err != nil {
...
func (s BigQueryClickhouseSuite) Test_BigQuery_Source_CDC_Validation() {
	...
	t.Run("QUERY replication mode", func(t *testing.T) {
		...
		keylessTableName := e2e.AddSuffix(s, "source_validation_no_pkey")
...
func (s BigQueryClickhouseSuite) Test_Types() {
	...
	source := s.Source().(*bigQuerySource)
	srcTable := e2e.AddSuffix(s, "test_types")
	...
	e2e.EnvWaitForEqualTablesWithNames(env, s, "all types replicated", srcTable, dstTable, "id")
...
func (s BigQueryClickhouseSuite) Test_JSON_Support() {
	...
	source := s.Source().(*bigQuerySource)
	srcTable := e2e.AddSuffix(s, "test_json")
	dstTable := srcTable + "_dst"
	...
	e2e.EnvWaitForEqualTablesWithNames(env, s, "JSON data replicated", srcTable, dstTable, "id,name")
...
func (s BigQueryClickhouseSuite) Test_GCS_Cleanup_After_Initial_Load() {
	...
	source := s.Source().(*bigQuerySource)
	srcTable := e2e.AddSuffix(s, "test_gcs_cleanup")
	dstTable := srcTable + "_dst"
	...
	e2e.EnvWaitForEqualTablesWithNames(env, s, "data replicated", srcTable, dstTable, "id")
	...
	e2e.EnvWaitFor(t, env, time.Second*30, fmt.Sprintf("GCS path %s to be cleaned up", gcsStagingPath), func() bool {
