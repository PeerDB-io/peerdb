...
func (s opensearchSuite) Test_Simple_QRep_Append() {
	srcTableName := e2e.AttachSchema(s, "os_simple_append")
	...
	qrepConfig := e2e.CreateQRepWorkflowConfig(s.t, "test_os_simple_qrep",
		srcTableName,
		srcTableName,
		query,
		s.Peer().Name,
	...
	e2e.EnvWaitFor(s.t, env, 10*time.Second, "waiting for OS to catch up", func() bool {
	...
	e2e.EnvWaitFor(s.t, env, 20*time.Second, "waiting for OS to catch up", func() bool {
...
func (s opensearchSuite) Test_Simple_QRep_Upsert() {
	srcTableName := e2e.AttachSchema(s, "os_simple_upsert")
