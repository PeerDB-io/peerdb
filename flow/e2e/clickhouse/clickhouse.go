...
func (s ClickHouseSuite) GetRows(table string, cols string) (*model.QRecordBatch, error) {
	...
	if s.cluster {
		ch9000, err := connclickhouse.Connect(s.t.Context(), nil, s.PeerForDatabase(peer.Config.(*protos.Peer_ClickhouseConfig).ClickhouseConfig.Database).GetClickhouseConfig())
		if err != nil {
			return nil, err
		}
		defer ch9000.Close()

		for idx, ch9k := range []clickhouse.Conn{ch, ch9000} {
			if idx == 0 {
				s.t.Log("Q9001", table)
			} else {
				s.t.Log("Q9000", table)
			...
			rows, err := ch9k.Query(
				s.t.Context(),
				fmt.Sprintf(`SELECT * FROM %s ORDER BY 1 SETTINGS use_query_cache = false`, table),
			)
			if err != nil {
				s.t.Log("ERROR", err)
				continue
		...
		for idx, ch9k := range []clickhouse.Conn{ch, ch9000} {
			if idx == 0 {
				s.t.Log("Q9001shard", table)
			} else {
				s.t.Log("Q9000shard", table)
			}
			rows, err := ch9k.Query(
				s.t.Context(),
				fmt.Sprintf(`SELECT * FROM %s_shard ORDER BY 1 SETTINGS use_query_cache = false`, table),
			)
			if err != nil {
				s.t.Log("ERROR", err)
				continue
		...
		raw := "_peerdb_raw_" + strings.TrimSuffix(table, "_dst") + "_" + s.Suffix()
		for idx, ch9k := range []clickhouse.Conn{ch, ch9000} {
			if idx == 0 {
				s.t.Log("Q9001raw", raw)
			} else {
				s.t.Log("Q9000raw", raw)
			}
			rows, err := ch9k.Query(
				s.t.Context(),
				fmt.Sprintf(`SELECT * FROM %s ORDER BY 1 SETTINGS use_query_cache = false`, raw),
			)
			if err != nil {
				s.t.Log("ERROR", err)
				continue
		...
		for idx, ch9k := range []clickhouse.Conn{ch, ch9000} {
			if idx == 0 {
				s.t.Log("Q9001rawshard", raw)
			} else {
				s.t.Log("Q9000rawshard", raw)
			}
			rows, err := ch9k.Query(
				s.t.Context(),
				fmt.Sprintf(`SELECT * FROM %s_shard ORDER BY 1 SETTINGS use_query_cache = false`, raw),
			)
			if err != nil {
				s.t.Log("ERROR", err)
				continue
