...
// Unbounded NUMERICs (no precision, scale specified) are mapped to String on CH if FF enabled, Decimal if not, test
func (s ClickHouseSuite) Test_Unbounded_Numeric_With_FF() {
	srcFullName := s.attachSchemaSuffix("unumeric_ff")
	dstTableName := "unumeric_ff"
	...
	_, err = s.Conn().Exec(context.Background(), fmt.Sprintf(`
	INSERT INTO %s(c) VALUES(%s);`, srcFullName, strings.Repeat("9", 78)))
	require.NoError(s.t, err)

	connectionGen := e2e.FlowConnectionGenerationConfig{
		FlowJobName:      s.attachSuffix("clickhouse_test_unbounded_numerics_ff"),
		TableNameMapping: map[string]string{srcFullName: dstTableName},
		Destination:      s.Peer().Name,
	}
	flowConnConfig := connectionGen.GenerateFlowConnectionConfigs(s.t)
	flowConnConfig.DoInitialSnapshot = true
	flowConnConfig.Env = map[string]string{"PEERDB_CLICKHOUSE_NUMERIC_AS_STRING": "true"}
	tc := e2e.NewTemporalClient(s.t)
	env := e2e.ExecutePeerflow(tc, peerflow.CDCFlowWorkflow, flowConnConfig, nil)
	e2e.SetupCDCFlowStatusQuery(s.t, env, flowConnConfig)
	...
	for _, row := range rows.Records {
		...
		require.Equal(s.t, strings.Repeat("9", 78), c, "expected unbounded NUMERIC to be 78 9s")
	}
...
func (s ClickHouseSuite) Test_Unbounded_Numeric_Without_FF() {
	srcFullName := s.attachSchemaSuffix("unumeric_no_ff")
	dstTableName := "unumeric_no_ff"
	...
	connectionGen := e2e.FlowConnectionGenerationConfig{
		FlowJobName:      s.attachSuffix("clickhouse_test_unbounded_numerics_no_ff"),
		TableNameMapping: map[string]string{srcFullName: dstTableName},
		Destination:      s.Peer().Name,
	}
	flowConnConfig := connectionGen.GenerateFlowConnectionConfigs(s.t)
	...
	for _, row := range rows.Records {
		...
		require.Equal(s.t, strings.Repeat("9", 78), c.String(), "expected unbounded NUMERIC to be 78 9s")
	}
