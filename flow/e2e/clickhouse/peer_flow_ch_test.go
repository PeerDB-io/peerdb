...
func TestPeerFlowE2ETestSuiteCH(t *testing.T) {
	e2eshared.RunSuite(t, SetupSuite, func(s PeerFlowE2ETestSuiteCH) {
		e2e.TearDownPostgres(s)

		if s.chHelper != nil {
			err := s.chHelper.Cleanup()
			if err != nil {
				s.t.Fatalf("failed to tear down Clickhouse: %v", err)
			}
		}

		err := s.connector.Close()
		if err != nil {
			s.t.Fatalf("failed to close Clickhouse connector: %v", err)
		}
	})
}
...
// 	tableName := "test_simple_flow_ch"
// 	srcTableName := s.attachSchemaSuffix(tableName)
// 	dstTableName := fmt.Sprintf("%s.%s", s.chHelper.testSchemaName, tableName)
...
// func (s PeerFlowE2ETestSuiteCH) Test_Flow_ReplicaIdentity_Index_No_Pkey() {
// 	env := e2e.NewTemporalTestWorkflowEnvironment(s.t)

// 	tableName := "test_replica_identity_no_pkey"
// 	srcTableName := s.attachSchemaSuffix(tableName)
// 	dstTableName := fmt.Sprintf("%s.%s", s.chHelper.testSchemaName, "test_replica_identity_no_pkey")
...
// 	count, err := s.chHelper.CountRows("test_replica_identity_no_pkey")
// 	require.NoError(s.t, err)
...
// 	tableName := "test_invalid_geo_ch_avro_cdc"
// 	srcTableName := s.attachSchemaSuffix(tableName)
...
// 		e2e.EnvWaitForEqualTables(env, s, "normalizing tx", "test_toast_ch_1", `id,t1,t2,k`)
// 		env.CancelWorkflow()
// 	}()
...
// 		e2e.EnvNoError(s.t, env, err)
// 		s.t.Log("Executed a transaction touching toast columns")
// 		e2e.EnvWaitForEqualTables(env, s, "normalizing tx", "test_toast_ch_3", `id,t1,t2,k`)
// 		env.CancelWorkflow()
// 	}()
...
// 		e2e.EnvNoError(s.t, env, err)
// 		s.t.Log("Executed a transaction touching toast columns")
// 		e2e.EnvWaitForEqualTables(env, s, "normalizing tx", "test_toast_ch_4", `id,t1,k`)
// 		env.CancelWorkflow()
// 	}()
...
// 		e2e.EnvWaitForEqualTables(env, s, "normalizing tx", "test_toast_ch_5", `id,t1,t2,k`)
// 		env.CancelWorkflow()
// 	}()
...
// 	// check if JSON on clickhouse side is a good JSON
// 	err = s.checkJSONValue(dstTableName, "c17", "sai", "1")
// 	require.NoError(s.t, err)

// 	// check if HSTORE on clickhouse is a good JSON
...
// 	srcTableName := s.attachSchemaSuffix(tableName)
// 	dstTableName := fmt.Sprintf("%s.%s", s.chHelper.testSchemaName, dstName)
...
// func (s PeerFlowE2ETestSuiteCH) Test_Supported_Mixed_Case_Table_CH() {
// 	env := e2e.NewTemporalTestWorkflowEnvironment(s.t)

// 	srcTableName := s.attachSchemaSuffix("testMixedCase")
// 	dstTableName := fmt.Sprintf("%s.%s", s.chHelper.testSchemaName, "testMixedCase")
