...
func (s ClickHouseSuite) Test_Addition_Removal() {
	...
	`, srcTableName))
	require.NoError(s.t, err)
	e2e.EnvWaitForEqualTables(env, s, "first insert", dstTableName, "id,key,updated_at")
	...
	`, addedSrcTableName))
	require.NoError(s.t, err)
	e2e.EnvWaitForEqualTables(env, s, "second insert to added table", dstTableName, "id,key,updated_at")
