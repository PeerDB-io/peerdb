...
func (s PeerFlowE2ETestSuitePG) Test_Types_QValue() {
	...
	srcTable := s.attachSchemaSuffix("test_types_q")
	dstTable := s.attachSchemaSuffix("test_types_q_dst")
...
func (s PeerFlowE2ETestSuitePG) Test_Types_PG() {
	...
	connectionGen := FlowConnectionGenerationConfig{
		...
		TableNameMapping: map[string]string{srcTableName.Deparse(): dstTableName.Deparse()},
		Destination:      s.Peer().Name,
	}
	...
	EnvWaitFor(s.t, env, 3*time.Minute, "normalize types", func() bool {
		err := s.comparePGTables(srcTableName.String(), dstTableName.String(), allCols)
		if err != nil {
			s.t.Log("mismatch", err)
		}
		return err == nil
	...
	var c36 string
	require.NoError(s.t, s.Conn().QueryRow(s.t.Context(), "select c36 from "+dstTableName.String()).Scan(&c36))
...
func (s PeerFlowE2ETestSuitePG) Test_NaN_Infinity_PG() {
	...
	EnvWaitFor(s.t, env, 3*time.Minute, "initial load nan/inf", func() bool {
		return s.comparePGTables(srcTableName.String(), dstTableName.String(), "id,f4::text,f8::text,n::text") == nil
	})
...
func (s PeerFlowE2ETestSuitePG) Test_PG_PG_Target_Foreign_Keys() {
	...
	config := &protos.FlowConnectionConfigs{
		...
		TableMappings: []*protos.TableMapping{
			{
				SourceTableIdentifier:      srcTableName1.Deparse(),
				DestinationTableIdentifier: dstTableName1.Deparse(),
			},
			{
				SourceTableIdentifier:      srcTableName2.Deparse(),
				DestinationTableIdentifier: dstTableName2.Deparse(),
			},
		},
		SourceName:        GeneratePostgresPeer(s.t).Name,
		MaxBatchSize:      100,
	...
	EnvWaitFor(s.t, env, 3*time.Minute, "wait for child table initial load", func() bool {
		return s.comparePGTables(srcTableName2.String(), dstTableName2.String(), "id,parent_id,val") == nil
	})
