...
func TestParseArgsAccordingTo(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			specs:   []ArgSpec{{"filter", ArgJSON, false}},
			rawArgs: []string{`{"name": "test"}`},
			wantLen: 1,
		},
		{
			name:    "string arg",
			specs:   []ArgSpec{{"field", ArgString, false}},
			rawArgs: []string{`"email"`},
			wantLen: 1,
		},
		{
			name:    "int arg",
			specs:   []ArgSpec{{"n", ArgInt, false}},
			rawArgs: []string{"10"},
			wantLen: 1,
		},
		{
			name:    "bool arg",
			specs:   []ArgSpec{{"enabled", ArgBool, false}},
			rawArgs: []string{"true"},
			wantLen: 1,
		},
		{
			name:    "optional arg present",
			specs:   []ArgSpec{{"filter", ArgJSON, false}, {"projection", ArgJSON, true}},
		...
		},
		{
			...
			specs:   []ArgSpec{{"filter", ArgJSON, false}, {"projection", ArgJSON, false}},
			rawArgs: []string{`{"filter": 1}`},
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ParseArgsAccordingTo(tt.specs, tt.rawArgs)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
