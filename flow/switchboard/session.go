...
func parseStartupConfig(params map[string]string) *startupConfig {
	cfg := &startupConfig{
		maxRows:      internal.PeerDBSwitchboardMaxRows(),
		maxBytes:     internal.PeerDBSwitchboardMaxBytes(),
		queryTimeout: time.Duration(internal.PeerDBSwitchboardQueryTimeoutSeconds()) * time.Second,
		writeTimeout: time.Duration(internal.PeerDBSwitchboardWriteTimeoutSeconds()) * time.Second,
