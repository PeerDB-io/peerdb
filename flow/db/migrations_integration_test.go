// Exercises the goose bootstrap against the refinery ledger by driving the
// Tilt stack's nexus and catalog containers (CI_NEXUS_CONTAINER and
// CI_CATALOG_CONTAINER), so it runs with the Postgres-source jobs.
//go:build postgres
