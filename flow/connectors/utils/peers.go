...
func CreatePeerNoValidate(
	...
	case protos.DBType_POSTGRES:
		...
		if !ok {
			return nil, status.Errorf(codes.InvalidArgument, wrongConfigResponse.Message)
		}
	...
	if _, err := pool.Exec(ctx, `
		...
		return nil, status.Errorf(codes.Internal, fmt.Sprintf("failed to upsert into peers table for %s peer %s: %s",
