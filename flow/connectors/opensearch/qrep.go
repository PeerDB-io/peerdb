...
func (osc *OpensearchConnector) SetupQRepMetadataTables(ctx context.Context,
...
func (osc *OpensearchConnector) SyncQRepRecords(ctx context.Context, config *protos.QRepConfig,
	...
	osBulkIndexer, err := opensearchutil.NewBulkIndexer(opensearchutil.BulkIndexerConfig{
	...
	if err != nil {
		...
		return 0, nil, fmt.Errorf("[os] failed to initialize bulk indexer: %w", err)
	}
	defer func() {
		if !bulkIndexerHasShutdown {
			if err := osBulkIndexer.Close(context.Background()); err != nil {
	...
	for qRecord := range stream.Records {
		...
		if err != nil {
			...
			return 0, nil, fmt.Errorf("[os] failed to json.Marshal record: %w", err)
		}

		if err := osBulkIndexer.Add(ctx, opensearchutil.BulkIndexerItem{
			Action:     actionIndex,
			DocumentID: docId,
			...
			return 0, nil, fmt.Errorf("[os] failed to add record to bulk indexer: %w", err)
		}
		if bulkIndexFatalError != nil {
			osc.logger.Error("[os] fatal error while indexing record", slog.Any("error", bulkIndexFatalError))
			return 0, nil, fmt.Errorf("[os] fatal error while indexing record: %w", bulkIndexFatalError)
		}
	...
	if err := stream.Err(); err != nil {
		osc.logger.Error("[os] failed to get record from stream", slog.Any("error", err))
		return 0, nil, fmt.Errorf("[os] failed to get record from stream: %w", err)
	}
	if err := osBulkIndexer.Close(ctx); err != nil {
		osc.logger.Error("[os] failed to close bulk indexer", slog.Any("error", err))
		return 0, nil, fmt.Errorf("[os] failed to close bulk indexer: %w", err)
	}
	...
	if err := osc.FinishQRepPartition(ctx, partition, config.FlowJobName, startTime); err != nil {
		osc.logger.Error("[os] failed to log partition info", slog.Any("error", err))
		return 0, nil, fmt.Errorf("[os] failed to log partition info: %w", err)
	}
