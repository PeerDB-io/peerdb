...
type QueryCDCReplicationState struct {
	...
	CursorText    string
	InflightState *model.QueryCDCInflightState
...
func (p *PostgresMetadata) GetQueryCDCReplicationState(
	...
	if err := p.pool.QueryRow(ctx,
		`SELECT cursor_text, inflight_state, last_attempt_at, last_synced_at, synced_batch_id, normalized_batch_id, last_normalized_at,
	...
	).Scan(&state.CursorText, &inflightJSON, &lastAttemptAt, &lastSyncedAt, &state.SyncedBatchID, &state.NormalizedBatchID, &lastNormalizedAt,
	...
	if len(inflightJSON) != 0 {
		state.InflightState = &model.QueryCDCInflightState{}
		if err := json.Unmarshal(inflightJSON, state.InflightState); err != nil {
			return QueryCDCReplicationState{}, fmt.Errorf("failed to decode in-flight state for %s: %w", sourceTableIdentifier, err)
		}
	}
...
// RecordQueryCDCAttempt records a failed poll attempt. It is intentionally
// called after failure: a worker dying mid-pull leaves the previous eligibility
// unchanged, so its replacement can resume immediately.
...
// normalize. inflightState is the next resumable Storage Read position and is
// cleared when the logical window completes. pullImmediately avoids applying
// the normal polling interval between slices of the same large window.
func (p *PostgresMetadata) RecordQueryCDCSync(
	...
	inflightState *model.QueryCDCInflightState, pullImmediately bool,
) error {
	var inflightJSON []byte
	var err error
	if inflightState != nil {
		inflightJSON, err = json.Marshal(inflightState)
		if err != nil {
			return fmt.Errorf("failed to encode in-flight state: %w", err)
		}
	}
	if _, err := p.pool.Exec(ctx, `
		...
		SET cursor_text = $3,
			...
			inflight_state = $6,
			last_attempt_at = CASE WHEN $7 THEN NULL ELSE $4 END,
			updated_at = now()
		WHERE flow_name = $1 AND source_table_identifier = $2
	`, jobName, sourceTableIdentifier, cursor, syncedAt, newBatchID, inflightJSON, pullImmediately); err != nil {
