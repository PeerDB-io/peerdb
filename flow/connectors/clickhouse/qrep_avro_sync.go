...
func NewClickhouseAvroSyncMethod(
	config *protos.QRepConfig,
	connector *ClickhouseConnector) *ClickhouseAvroSyncMethod {
...
func (s *ClickhouseAvroSyncMethod) SyncQRepRecords(
	...
	log.WithFields(log.Fields{
		"flowName":    config.FlowJobName,
		"partitionID": partition.PartitionId,
	}).Infof("Put file to stage in Avro sync for clickhouse")
...
func (s *ClickhouseAvroSyncMethod) putFileToStage(localFilePath string, stage string) error {
	if localFilePath == "" {
...
func (sc *ClickhouseConnector) GetCopyTransformation(
	dstTableName string,
) (*CopyInfo, error) {
...
func GenerateMergeCommand(
	...
	// all cols are acquired from Clickhouse schema, so let us try to make upsert key cols match the case
