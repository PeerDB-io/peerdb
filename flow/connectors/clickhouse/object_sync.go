...
func (c *ClickHouseConnector) SyncQRepObjects(
	...
	for object := range stream.Objects {
		...
		c.logger.Info("Processing downloadable object",
...
func (c *ClickHouseConnector) insertFromURLWithMapping(
	...
	c.logger.Info("Executing URL table function query",
		slog.String("url", object.URL),
		slog.String("format", format),
		slog.String("query", query))

	// Execute the query and return records count
	// Note: ClickHouse doesn't easily return inserted row count from url() functions,
	// so we estimate based on object size or return the object size as a proxy
	if err := c.exec(ctx, query); err != nil {
		return 0, err
	}

	// For now, we return object.Size as an estimate of records processed
	// In a real implementation, you might want to query the table before/after
	// or use other mechanisms to get the actual record count
