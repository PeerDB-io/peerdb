...
func (s *ClickHouseAvroSyncMethod) pushDataToS3ForSnapshot(
	...
	createChunkedSubstream := func(done *atomic.Bool) (*model.QRecordStream, *shared.AvroChunkSizeTracker) {
		substream := model.NewQRecordStream(0)
		substream.SetSchema(schema)
		sizeTracker := shared.AvroChunkSizeTracker{TrackUncompressed: trackUncompressed}
		go func() {
			recordsDone := true
