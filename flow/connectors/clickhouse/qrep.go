...
func (c *ClickhouseConnector) SyncQRepRecords(
	...
	case protos.QRepSyncMode_QREP_SYNC_MODE_MULTI_INSERT:
		return 0, fmt.Errorf("multi-insert sync mode not supported for clickhouse")
...
func (c *ClickhouseConnector) createQRepMetadataTable(createMetadataTableTx *sql.Tx) error {
	// Define the schema
	schemaStatement := `
...
func (c *ClickhouseConnector) ConsolidateQRepPartitions(config *protos.QRepConfig) error {
	...
	switch syncMode {
	case protos.QRepSyncMode_QREP_SYNC_MODE_MULTI_INSERT:
		return fmt.Errorf("multi-insert sync mode not supported for clickhouse")
	case protos.QRepSyncMode_QREP_SYNC_MODE_STORAGE_AVRO:
