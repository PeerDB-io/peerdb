...
func (c *ClickHouseConnector) CreateRawTable(ctx context.Context, req *protos.CreateRawTableInput) (*protos.CreateRawTableOutput, error) {
	...
	useJSON, err := peerdbenv.PeerDBClickHouseUseExperimentalJSONType(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("unable to get fetch status of setting PEERDB_CLICKHOUSE_USE_EXPERIMENTAL_JSON_TYPE: %w", err)
	}
	if useJSON {
		peerdbDataType = "JSON"
