...
// structuredTableMapping is a MongoDB table mapping with structured ingestion enabled for the given columns.
func structuredTableMapping(source string, columns []*protos.ColumnSetting) *protos.TableMapping {
...
func TestGetTableSchemaStructured(t *testing.T) {
	...
	schemas, err := (&MongoConnector{}).GetTableSchema(t.Context(), nil, shared.InternalVersion_Latest, protos.TypeSystem_Q,
		[]*protos.TableMapping{
			structuredTableMapping(structuredTable, structuredTestColumns()),
	...
	_, err = (&MongoConnector{}).GetTableSchema(t.Context(), nil, shared.InternalVersion_Latest, protos.TypeSystem_Q,
		[]*protos.TableMapping{structuredTableMapping(structuredTable,
			[]*protos.ColumnSetting{{SourceName: "location", DestinationType: "Point"}})})
