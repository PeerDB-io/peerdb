...
func (c *MongoConnector) PullQRepRecords(
	...
	for cursor.Next(ctx) {
		...
		record, bytes, err := qValuesFromDoc(doc)
		if err != nil {
...
func qValuesFromDoc(doc bson.D) ([]types.QValue, int64, error) {
	var qValues []types.QValue
	var size int64

	// process document key
	var qValueIdString types.QValueString
	var err error
	for _, v := range doc {
		if v.Key == DefaultDocumentKeyColumnName {
			qValueIdString, err = qValueStringFromKey(v.Value)
			if err != nil {
				return nil, 0, fmt.Errorf("failed to convert _id to string: %v", err)
			}
			break
		}
	}
	qValues = append(qValues, qValueIdString)

	// process full document
	qValueJSON, err := qValueJSONFromDoc(doc)
	if err != nil {
		return nil, 0, err
	}
	qValues = append(qValues, qValueJSON)

	// TODO: more accurate size calculation
	size += int64(len(qValueJSON.Val))
