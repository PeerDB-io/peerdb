...
// so the driver sends it as maxTimeMS over the wire. Otherwise server-
// side default maxTimeMS is used which can sometimes be too short.
func createChangeStream(client *mongo.Client, parent context.Context, pipeline mongo.Pipeline, opts *options.ChangeStreamOptionsBuilder,
