...
func TestBuildMinMaxQuery(t *testing.T) {
	...
	systemTime := crdbHLC{WallNanos: 1712345678901234567, Logical: 1}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expected, buildMinMaxQuery(table, "id", systemTime, tc.resuming))
		})
	}
}
...
func TestSnapshotSystemTime(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, crdbHLC{WallNanos: 1712345678901234567, Logical: 1}, systemTime)
	require.Equal(t, "1712345678901234567.0000000001", systemTime.String())

	// wall-only snapshot names parse with a zero logical component
	systemTime, err = c.snapshotSystemTime(t.Context(), &protos.QRepConfig{SnapshotName: "1712345678901234567"})
	require.NoError(t, err)
	require.Equal(t, crdbHLC{WallNanos: 1712345678901234567}, systemTime)
...
func TestClassifySnapshotReadError(t *testing.T) {
	gcErr := fmt.Errorf("failed to execute query: %w", &pgconn.PgError{
		Severity: "ERROR",
		Message:  "batch timestamp 1712345678.0000000000,0 must be after replica GC threshold 1712349999.0000000000,0",
	})
	err := classifySnapshotReadError(gcErr)
	var appErr *temporal.ApplicationError
	require.ErrorAs(t, err, &appErr)
	require.True(t, appErr.NonRetryable(), "GC threshold errors can never succeed on retry")
	require.Equal(t, exceptions.ApplicationErrorTypeIrrecoverableInvalidSnapshot.String(), appErr.Type())
	require.ErrorContains(t, err, "resync the mirror")

	// anything else passes through untouched for Temporal activity retries
	plain := errors.New("connection reset by peer")
	require.Equal(t, plain, classifySnapshotReadError(plain))
	require.NoError(t, classifySnapshotReadError(nil))
}
