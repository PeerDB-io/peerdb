...
// never succeed and the mirror needs a resync. This mirrors the postgres
// invalid-snapshot handling; shared.WrapError re-raises the ApplicationError
// even when it gets wrapped along the way.
func classifySnapshotReadError(err error) error {
	if err != nil && isCursorTooOldError(err) {
...
func (c *CockroachDBConnector) GetQRepPartitions(
	...
	resuming := last != nil && last.Range != nil
	if resuming {
	...
	// numPartitions was not overridden, derive it from the row count
	if numPartitions == 0 {
		var totalRows int64
		if resuming {
			// resuming mid load is only reachable through the standalone QRep
			// flow, a deprecated feature, so it is kept in its own branch to be
			// easy to remove; the rows past lastRangeEnd cannot be estimated
			// from table statistics, count them exactly instead. Both this
			// count and the min/max below read AS OF the same system time, so
			// they stay consistent without sharing a scan.
			countQuery := fmt.Sprintf("SELECT COUNT(*) FROM %s AS OF SYSTEM TIME '%s' WHERE %s > $1",
				parsedWatermarkTable.String(), systemTime, common.QuoteIdentifier(config.WatermarkColumn))
			if err := c.conn.QueryRow(ctx, countQuery, lastRangeEnd).Scan(&totalRows); err != nil {
				return nil, classifySnapshotReadError(fmt.Errorf("failed to query for remaining rows: %w", err))
			}
		} else {
			// starting from the beginning: use the statistics based estimate,
			// fall back to a precise count when it is unusable
			estimate, err := c.tableRowEstimate(ctx, parsedWatermarkTable)
			if err != nil || estimate <= 0 {
				// a NULL estimate scans as 0 without an error, so this branch covers
				// both a failed estimate and a genuinely empty (or unanalyzed) table
				c.logger.Warn("[cockroachdb] failed to estimate row count or table is empty, falling back to precise count",
					slog.Any("error", err))
				countQuery := fmt.Sprintf("SELECT COUNT(*) FROM %s AS OF SYSTEM TIME '%s'", parsedWatermarkTable.String(), systemTime)
				if err := c.conn.QueryRow(ctx, countQuery).Scan(&estimate); err != nil {
					return nil, classifySnapshotReadError(fmt.Errorf("failed to query for total rows: %w", err))
				}
			}
			totalRows = estimate
		}
		...
		if totalRows == 0 {
			c.logger.Warn("no records to replicate, only using 1 partition")
			numPartitions = 1
		} else {
	...
	minmaxQuery := buildMinMaxQuery(parsedWatermarkTable, config.WatermarkColumn, systemTime, resuming)
	var queryArgs []any
	if resuming {
	...
	if err := c.conn.QueryRow(ctx, minmaxQuery, queryArgs...).Scan(&minVal, &maxVal); err != nil {
		return nil, classifySnapshotReadError(fmt.Errorf("failed to query for min/max: %w", err))
	}

	switch minVal.(type) {
	case nil, int16, int32, int64, uint8, uint16, uint32, uint64, time.Time:
	default:
...
func (c *CockroachDBConnector) PullQRepRecords(
	...
	if err != nil {
		return 0, 0, classifySnapshotReadError(fmt.Errorf("failed to execute query: %w", err))
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return totalRecords, totalBytes, classifySnapshotReadError(fmt.Errorf("row iteration failed: %w", err))
	}
...
func (c *CockroachDBConnector) mapRowToQRecord(
	...
	for i, fd := range fds {
		...
		case pgtype.JSONOID, pgtype.JSONBOID:
			...
			if jsonVal == "null" {
				// a JSON null is indistinguishable from SQL NULL in changefeed
				// envelopes (verified live: both emit a JSON null), so the
				// snapshot path maps it to NULL too, keeping a row identical
				// whichever path delivered it
				record[i] = types.QValueNull(types.QValueKindJSON)
			} else {
...
func buildMinMaxQuery(
	...
	fmt.Fprintf(&sb, "SELECT MIN(%[1]s),MAX(%[1]s) FROM %s AS OF SYSTEM TIME '%s'", quotedColumn, table.String(), systemTime)
	if resuming {
