...
const (
	// changefeedResolvedInterval is the fixed resolved-timestamp cadence.
	// Every PullRecords call opens a fresh changefeed at the last resolved
	// timestamp, so events committed after it are replayed at the start of
	// the next batch even on the happy path; a one second cadence bounds
	// that replay to about a second of events per sync interval.
	changefeedResolvedInterval = time.Second

	// changefeed reconnect tuning: transient session failures are retried
	// with exponential backoff before PullRecords returns the error and the
	// activity-level retry policy takes over
	changefeedMaxRetries     = uint32(5)
	changefeedRetryBaseDelay = 500 * time.Millisecond
...
func buildChangefeedStatement(tables []*common.QualifiedTable, opts changefeedOptions) (string, error) {
	...
	intervalMs := max(opts.ResolvedInterval, changefeedResolvedInterval).Milliseconds()
...
// newChangefeedTableSchema builds the cached per-table schema for envelope
// decoding. Excluded columns are seeded into the ignored set: the cached
// schema already has them removed, so without this they would look like
// newly added columns and re-enter through a schema delta.
func newChangefeedTableSchema(schema *protos.TableSchema, exclude map[string]struct{}) *changefeedTableSchema {
...
func isPermanentChangefeedError(err error) bool {
	...
	if pgErr, ok := errors.AsType[*pgconn.PgError](err); ok {
		switch pgErr.Code {
		case pgerrcode.UndefinedTable, pgerrcode.SyntaxError, pgerrcode.InsufficientPrivilege:
			// 42501: the user lacks CHANGEFEED (or SELECT) on a watched table;
			// privileges do not come back on their own, so burning the in-loop
			// retry budget only delays the actionable error. Temporal still
			// retries the activity, so a later GRANT recovers the mirror.
			return true
		default:
