...
func TestUnknownColumns(t *testing.T) {
	...
	// excluded columns are stripped from the cached schema, so without the
	// seeded ignore set they would masquerade as newly added columns and
	// re-enter the destination through a schema delta
	excluding := newChangefeedTableSchema(&protos.TableSchema{
		Columns: []*protos.FieldDescription{
			{Name: "id", Type: string(types.QValueKindInt64)},
		},
	}, map[string]struct{}{"secret": {}})
	require.Empty(t, excluding.unknownColumns(map[string]json.RawMessage{
		"id":     json.RawMessage(`1`),
		"secret": json.RawMessage(`"s"`),
	}))
	require.Equal(t, []string{"added"}, excluding.unknownColumns(map[string]json.RawMessage{
		"id":     json.RawMessage(`1`),
		"secret": json.RawMessage(`"s"`),
		"added":  json.RawMessage(`2`),
	}))
}
...
func TestChangefeedErrorClassification(t *testing.T) {
	...
	// missing CHANGEFEED/SELECT privilege recurs deterministically on every
	// reconnect; surface it immediately instead of burning the retry budget
	insufficientPrivilege := &pgconn.PgError{
		Code:    pgerrcode.InsufficientPrivilege,
		Message: "user testuser requires the CHANGEFEED privilege on all target tables to be able to run an enterprise changefeed",
	}
	require.True(t, isPermanentChangefeedError(insufficientPrivilege))
	require.True(t, isPermanentChangefeedError(fmt.Errorf("failed to create changefeed: %w", insufficientPrivilege)))
...
func TestProcessChangefeedRowFailureThenReplay(t *testing.T) {
	ctx := t.Context()
