...
func TestCrdbTypeToQValueKind(t *testing.T) {
	testCases := []struct {
		...
		{"oid", "oid", types.QValueKindUInt32},
...
func TestCrdbOIDToQValueKind(t *testing.T) {
	testCases := []struct {
		...
		{pgtype.OIDOID, types.QValueKindUInt32},
...
func TestQValueFromCrdbValue(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "oid as uint32",
			field: field(types.QValueKindUInt32), value: uint32(42),
			expected: types.QValueUInt32{Val: 42},
		},
		...
		{
			name:  "time 24:00 clamps into the day like the changefeed path",
			field: field(types.QValueKindTime),
			value: pgtype.Time{Microseconds: 24 * 3600 * 1000000, Valid: true},
			expected: types.QValueTime{
				Val: 23*time.Hour + 59*time.Minute + 59*time.Second + 999999*time.Microsecond,
			},
		},
		{
			name:  "inet",
		...
		{
			name:  "inet host address drops the full mask like the changefeed path",
			field: field(types.QValueKindINET), value: "192.168.1.5/32",
			expected: types.QValueINET{Val: "192.168.1.5"},
		},
		{
			name:  "inet prefix host address renders bare",
			field: field(types.QValueKindINET), value: netip.MustParsePrefix("192.168.1.5/32"),
			expected: types.QValueINET{Val: "192.168.1.5"},
		},
		{
			name:  "inet prefix subnet keeps the mask",
			field: field(types.QValueKindINET), value: netip.MustParsePrefix("192.168.1.0/24"),
			expected: types.QValueINET{Val: "192.168.1.0/24"},
		},
		{
			name:  "inet ipv6 host address drops the full mask",
			field: field(types.QValueKindINET), value: "::1/128",
			expected: types.QValueINET{Val: "::1"},
		},
		{
			name:  "array int64",
