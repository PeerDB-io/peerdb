...
func (c *CockroachDBConnector) PullRecords(
	...
	for source, target := range req.TableNameMapping {
		...
		if schema, ok := req.TableNameSchemaMapping[target.Name]; ok {
			state.schemas[source] = newChangefeedTableSchema(schema, target.Exclude)
		}
	}
	...
	for {
		...
		attempt++
		if attempt > changefeedMaxRetries {
			return fmt.Errorf("changefeed failed after %d retries: %w", changefeedMaxRetries, err)
		}
		delay := changefeedBackoff(changefeedRetryBaseDelay, attempt)
...
func (c *CockroachDBConnector) runChangefeedSession(
	...
	if c.historyProtectionChecked.CompareAndSwap(false, true) {
		...
		// longer needs protection from garbage collection; see the field
		// declaration for why this check never needs resetting
...
func (c *CockroachDBConnector) emitSchemaDelta(
	...
	// the schema is read AS OF SYSTEM TIME at the row's commit timestamp, so
	// the added columns get the types the row was written with even when more
	// DDL has run since. The timestamp is always readable: it lies at or ahead
	// of the changefeed cursor, which itself must be inside the GC window for
	// the feed to run at all.
	freshSchemas, err := c.getTableSchema(ctx, state.req.Env, protos.TypeSystem_Q,
		[]*protos.TableMapping{{SourceTableIdentifier: source}}, asOf)
	if err != nil {
