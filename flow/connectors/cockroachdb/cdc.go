...
func (c *CockroachDBConnector) SetupReplication(
	...
	if shouldProtectSnapshotHistory(c.Config, req.DoInitialSnapshot) {
		c.protectSnapshotHistory(ctx, req.FlowJobName, systemTime)
	}
...
func (c *CockroachDBConnector) handleResolved(
	...
	otelManager.Metrics.CockroachDBResolvedLagGauge.Record(ctx, time.Since(time.Unix(0, wallNanos)).Seconds())
