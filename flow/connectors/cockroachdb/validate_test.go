...
func TestValidateGCTTLFloor(t *testing.T) {
	require.NoError(t, validateGCTTLFloor(`"public"."users"`, "RANGE default", gcTTLFloorSeconds))
	require.NoError(t, validateGCTTLFloor(`"public"."users"`, "RANGE default", 90000))

	err := validateGCTTLFloor(`"public"."users"`, `TABLE defaultdb.public.users`, gcTTLFloorSeconds-1)
	require.Error(t, err)
	...
	require.ErrorContains(t, err, `ALTER TABLE "public"."users" CONFIGURE ZONE USING gc.ttlseconds`)

	// a partition override cannot be raised through the table's zone config:
	// the hint must point at the partition instead
	err = validateGCTTLFloor(`"public"."events"`, `PARTITION archived OF TABLE "public"."events"`, 300)
	require.Error(t, err)
	require.ErrorContains(t, err, `configured on PARTITION archived OF TABLE "public"."events"`)
	require.ErrorContains(t, err, `ALTER PARTITION archived OF TABLE "public"."events" CONFIGURE ZONE USING`)
}
...
func TestHistoryRetentionLookupQuery(t *testing.T) {
	...
	// straight to CANCEL JOB. The primary lookup must read crdb_internal.jobs,
	// not SHOW JOBS: SHOW JOBS middle-truncates descriptions around 69
	// characters (verified on v24.1.30, v25.4.14 and v26.2.5), so a suffix
	// match against it silently misses every flow name long enough to push
	// the description past the truncation threshold
	require.Equal(t,
	...
	// the fallback (for sessions that cannot read crdb_internal.jobs) is the
	// same match against SHOW JOBS and differs from the primary only in the
	// source relation
	require.Equal(t,
		"SELECT job_id FROM [SHOW JOBS] WHERE job_type = 'HISTORY RETENTION'"+
			" AND status = 'running' AND right(description, length($1::text)) = $1::text LIMIT 1",
		historyRetentionLookupFallbackQuery)
	require.NotContains(t, historyRetentionLookupFallbackQuery, "LIKE")
	require.Equal(t, historyRetentionLookupFallbackQuery,
		strings.Replace(historyRetentionLookupQuery, "crdb_internal.jobs", "[SHOW JOBS]", 1))
}
...
func TestShouldExtendSnapshotHistory(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "initial snapshot pull of a mirror",
			qrep: &protos.QRepConfig{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expected, shouldExtendSnapshotHistory(tc.qrep))
		})
	}
}
