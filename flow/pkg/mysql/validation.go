...
func CheckMySQL8BinlogSettings(conn *client.Conn, logger log.Logger, requireRowMetadata bool) error {
	...
	if binlogRowMetadata != "FULL" {
		if isAzure, err := IsAzureFlexibleServer(conn); err == nil && isAzure && !requireRowMetadata {
