    ...
    pub async fn query(conn: MyClient, query: String) -> PgWireResult<Self> {
        ...
        if let Some(first) = recv.recv().await {
            match first {
                ...
                client::Response::Schema(schema) => {
                    Ok(MyRecordStream {
                        schema: MySchema::from_columns(&schema),
...
impl Stream for MyRecordStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match Pin::new(row_stream).poll_next(cx) {
            Poll::Ready(Some(client::Response::Row(ref row))) => {
                let values = mysql_row_to_values(row);
                let record = Record { values, schema };
                Poll::Ready(Some(Ok(record)))
            }
            Poll::Ready(Some(client::Response::Schema(..))) => {
                Poll::Ready(Some(Err(PgWireError::ApiError("second schema received".into()))))
            }
