...
impl From<&CockroachDbConfig> for PostgresConfig {
    fn from(config: &CockroachDbConfig) -> Self {
        PostgresConfig {
            ...
            // CockroachDB peers default to TLS on; the Postgres executor's
            // equivalent is an explicit disable_tls value
            disable_tls: Some(config.disable_tls),
            skip_cert_verification: config.skip_cert_verification,
            client_tls: config.client_tls.clone(),
            ..Default::default()
        }
    }
}
