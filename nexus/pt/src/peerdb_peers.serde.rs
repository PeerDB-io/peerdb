...
impl serde::Serialize for PostgresPeerConfigs {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("peerdb_peers.PostgresPeerConfigs", len)?;
        if !self.configs.is_empty() {
...
impl<'de> serde::Deserialize<'de> for PostgresPeerConfigs {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = PostgresPeerConfigs;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct peerdb_peers.PostgresPeerConfigs")
            }

            fn visit_map<V>(self, mut map: V) -> std::result::Result<PostgresPeerConfigs, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                ...
                Ok(PostgresPeerConfigs {
                    configs: configs__.unwrap_or_default(),
                })
            }
        }
        deserializer.deserialize_struct("peerdb_peers.PostgresPeerConfigs", FIELDS, GeneratedVisitor)
    }
}
