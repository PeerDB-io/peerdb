...
export const IsQueuePeer = (peerType?: DBType): boolean => {
  return (!!peerType &&
          (peerType === DBType.KAFKA || peerType === DBType.PUBSUB ||
...
const ValidSchemaQualifiedTarget = (peerType: DBType,
...
const CDCCheck = (flowJobName: string, rows: TableMapRow[], config: CDCConfig,
...
const validateSchemaQualification = (tableMapping: (TableMapping|undefined)[],
                                     destinationType: DBType): string => {
  for (const table of tableMapping) {
    if (!ValidSchemaQualifiedTarget(destinationType,
                                    table!.destinationTableIdentifier)) {
      return `Destination table ${
          table?.destinationTableIdentifier} should be schema qualified`;
    }
  }
  return '';
};
...
const validateCDCFields =
    (tableMapping: (TableMapping|undefined)[], config: CDCConfig,
     destinationType: DBType): string|undefined => {
      const tableQualificationErr =
          validateSchemaQualification(tableMapping, destinationType);
...
const validateQRepFields =
    (query: string, config: QRepConfig): string|undefined => {
      if (query.length < 5) {
...
export const reformattedTableMapping =
    (tableMapping: TableMapRow[]): TableMapping[] => {
...
export const handleCreateCDC = async (
    flowJobName: string, rows: TableMapRow[], config: CDCConfig,
    destinationType: DBType, setLoading: Dispatch<SetStateAction<boolean>>,
    route: RouteCallback) => {
...
export const handleCreateQRep = async (
    flowJobName: string, query: string, config: QRepConfig,
    destinationType: DBType, setLoading: Dispatch<SetStateAction<boolean>>,
    route: RouteCallback, xmin?: boolean) => {
  const flowNameValid = flowNameSchema.safeParse(flowJobName);
  if (!flowNameValid.success) {
  ...
  if (xmin == true) {
    ...
    config.query =
        `SELECT * FROM ${quotedWatermarkTable(config.watermarkTable)}`;
    query = config.query;
    config.initialCopyOnly = false;
  }

  if (config.writeMode?.writeType == QRepWriteType.QREP_WRITE_MODE_UPSERT &&
  ...
  if (!ValidSchemaQualifiedTarget(destinationType,
    ...
    notifyErr(`Destination table should be schema qualified for ${
        DBTypeToGoodText(destinationType)} targets`);
    return;
  }
...
export const fetchSchemas = async (peer_name: string) => {
  const schemasRes: PeerSchemasResponse =
      await fetch(
...
const getDefaultDestinationTable =
    (peerType: DBType, schemaName: string, tableName: string) => {
      if (peerType.toString() == 'BIGQUERY' ||
      ...
      if (peerType.toString() == 'CLICKHOUSE' ||
      ...
      if (peerType.toString() == 'EVENTHUBS' ||
...
export const fetchTables =
    async (peerName: string, schemaName: string, targetSchemaName: string,
           peerType?: DBType) => {
  if (schemaName.length === 0)
    return [];
...
export const fetchColumns =
    async (peerName: string, schemaName: string, tableName: string,
...
export const fetchPublications = async (peerName: string) => {
  const publicationsRes: UPublicationsResponse =
      await fetch('/api/peers/publications', {
        method : 'POST',
        body : JSON.stringify({
          peerName,
        }),
