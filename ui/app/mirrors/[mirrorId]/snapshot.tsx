...
import {
    CloneTableSummary, SnapshotStatus
...
export class TableCloneSummary {
  ...
  export function summarizeTableClone(clone: CloneTableSummary): TableCloneSummary {
  ...
  export const SnapshotStatusTable = ({ status }: SnapshotStatusProps) => {
    ...
    return (
      <div style={{ marginTop: '2rem', display:'flex', flexDirection:'column', rowGap:'1rem' }}>
        <Table
        title={<Label variant='headline'>Currently Fetching</Label>}
        />
        <Table
          title={<Label variant='headline'>Currently Syncing</Label>}
          toolbar={{
            left: (
        ...
        >
          {displayedRows.map((clone, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {getStatus(clone.cloneTableSummary)}
              </TableCell>
              {clone.cloneTableSummary.fetchCompleted ? <TableCell>
              ...
              <TableCell>{clone.cloneTableSummary.fetchCompleted ? clone.cloneTableSummary.numRowsSynced:0}</TableCell>
              {clone.cloneTableSummary.fetchCompleted? <TableCell>
                <Label>
