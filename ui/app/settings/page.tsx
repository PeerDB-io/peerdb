...
const ApplyModeIconWithTooltip = ({ applyMode }: { applyMode: number }) => {
    ...
    switch (applyMode) {
        ...
        case 2:
            ...
            break;
        case 3:
            tooltipText = 'Changes to this configuration will apply after server restart.';
            iconName = 'restart_alt';
            break;
        case 4:
            tooltipText = 'Changes to this configuration will apply only to new mirrors';
            iconName = 'new_window';
            break;
        default:
...
const DynamicSettingItem = ({ setting, onSettingUpdate }: { setting: dynamic_settings, onSettingUpdate: () => void }) => {
    ...
    return (
        <TableRow key={setting.id}>
            ...
            <TableCell style={{ width: '10%' }}>{setting.config_default_value || 'N/A'}</TableCell>
            <TableCell style={{ width: '35%' }}>{setting.config_description || 'N/A'}</TableCell>
            <TableCell style={{ width: '10%' }}>
        ...
        <div>
            ...
            >
                {displayedSettings.map((setting) => (
                    <DynamicSettingItem key={setting.id} setting={setting} onSettingUpdate={fetchSettings} />
                ))}
            </Table>
        </div>
    );
